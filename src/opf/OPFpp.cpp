...
namespace opf
{
  ...
  void opf_OPFTraining(Subgraph& sg)
  {
    ...
    // initialization
    PriorityHeap Q(sg.nodes.size(), pathval);
  ...
  Subgraph *opf_OPFSemiLearning(Subgraph& sg, Subgraph& nonsg, Subgraph& sgeval)
  {
    ...
    PriorityHeap Q(merged.nnodes, pathval);
  ...
  void opf_OPFClustering(Subgraph& sg)
  {
    ...
    float tmp;
    std::vector<float> pathval(sg.nodes.size());
    PriorityHeap Q(sg.nodes.size(), pathval, PriorityHeap::Policy::MAX_VALUE);
    ...
    while (!Q.IsEmpty())
    {
      ...
      {
        if (Q.GetColor(q_neighbor) != BLACK)
        {
  ...
  void opf_MSTPrototypes(Subgraph& sg)
  {
    ...
    while (!Q.IsEmpty())
    {
      ...
      for (q = 0; q < static_cast<int>(sg.nodes.size()); q++)
      {
        if (Q.GetColor(q) != PriorityHeap::BLACK)
        {
          if (p != q)
          {
            if (!opf_PrecomputedDistance)
